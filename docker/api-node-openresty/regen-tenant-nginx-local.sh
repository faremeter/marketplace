...
CONFIG_ALLOW_CIDR="${CONFIG_ALLOW_CIDR:-172.16.0.0/12}"
...
if [[ -f "$CONFIG_FILE" ]] && [[ -s "$CONFIG_FILE" ]]; then
  ...
  if [[ -n "$TENANT_ENTRIES" ]]; then
    while read -r domain; do
      ...
      if ! is_valid_server_name "$domain"; then
        echo "Invalid tenant domain in $CONFIG_FILE: $domain" >&2
        exit 1
      fi

      if [[ -n "$TENANT_SLUG" ]] && ! is_valid_gateway_slug "$TENANT_SLUG"; then
        echo "Invalid gateway_slug for tenant $domain: $TENANT_SLUG" >&2
        exit 1
      fi
