...
async function ensureSupportedToken() {
  ...
  await db
    ...
    .values({
      symbol: "USDC",
      mint_address: SOLANA_DEVNET_USDC,
...
async function useOnlyLocalDevnetUsdc(tenantId: number) {
  await db
    .deleteFrom("token_prices")
...
async function main() {
  ...
  // create the wallet via the public route, but we complete the EVM-only
  // funding check here as a deterministic bootstrap step.
  ...
  await useOnlyLocalDevnetUsdc(tenant.id);
