...
function applyExtraDirectives(
  locationsConf: string,
  directives: string[],
): string {
  if (directives.length === 0) return locationsConf;

  const rendered = directives.map((directive) => `  ${directive}`).join("\n");
  return locationsConf.replaceAll(
    "\n\n    access_by_lua_block",
    `\n${rendered}\n\n    access_by_lua_block`,
  );
}
...
export async function buildNodeConfig(nodeId: number) {
  ...
  for (const tenant of tenants) {
    ...
    gateway[gatewaySlug] = {
      spec,
      locationsConf: applyExtraDirectives(locationsConf, extraDirectives),
