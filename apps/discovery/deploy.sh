...
BLUE_DIR="/home/admin/discovery-blue"
GREEN_DIR="/home/admin/discovery-green"
SYMLINK_DIR="/home/admin/discovery"
...
step::10::verify-environment() {
    ...
    if [ ! -f "$SCRIPT_DIR/package.json" ]; then
        log::fatal "Must run from discovery directory"
    fi
...
step::50::deploy() {
    ...
    for_each_stack "switching release and restarting..." '
        ...
        remote::exec "sudo systemctl restart discovery"
    '
}

step::60::verify() {
    for_each_stack "verifying service..." 'remote::exec "sudo systemctl status discovery --no-pager" || true'
}
