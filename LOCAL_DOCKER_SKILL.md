...
## Required Local Keypairs

Create local Solana devnet keypairs before starting the stack:
...
```text
keypairs/facilitator-devnet.json
```

The smoke-test client keypair is used by the local smoke test:

```text
keypairs/client-devnet.json
```

Fund both wallets on Solana devnet. The facilitator needs devnet SOL for gas.
The client needs devnet SOL and devnet USDC for paid smoke requests.

```bash
solana airdrop --url devnet 1 <wallet-address>
```

Use Circle's faucet for Solana devnet USDC.
...
Check whether the facilitator and client wallets have enough local payment
funding:
...
This prints the facilitator and client wallet addresses plus SOL/USDC balances.
If funding is missing, fund the exact addresses shown by the command.

## Run The Smoke Test

After the stack is up, seeded, and funded:
...
- creates a new paid endpoint through the control plane
- verifies unpaid requests return `402`
- pays through both API nodes with Solana devnet USDC
...
When overriding proxy ports, keep smoke-test URLs and advertised proxy resource
URLs aligned with the same values.
...
8. Run `make local-smoke`.
9. Report any failing command with the relevant service logs.
