...
await t.test("gateway-nginx marketplace prototype", async (t) => {
  ...
  await t.test(
    ...
    async (t) => {
      ...
      t.equal(cap.captured, true, "capture must be recorded");
  ...
  await t.test(
    "free /health endpoint passes through without sidecar involvement",
    async (t) => {
      cb.reset();
      ...
      // Wait briefly to confirm no async sidecar activity fires.
      await new Promise((r) => setTimeout(r, 500));

      t.equal(cb.x402VerifyCount, 0, "sidecar verify must not fire");
      t.equal(cb.x402SettleCount, 0, "sidecar settle must not fire");
      t.equal(cb.captures.size, 0, "no captures must be recorded");
      t.end();
    },
  );

  // -- MPP charge: settles at access time (one-phase) --

  await t.test(
    "MPP charge: settle at access, capture at log phase",
    async (t) => {
      cb.reset();
      ...
      t.equal(res.status, 200);
      t.ok(cb.mppSettleCount > 0, "MPP settle must fire at access time");
      t.equal(
        cb.x402VerifyCount,
        0,
